...
export default function BoutiqueOrders() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-display text-3xl text-[#F8F8F2] tracking-wider">Ventes</h1>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="card-gym text-center">
          <ShoppingBag className="w-5 h-5 text-[#3B82F6] mx-auto mb-1" />
          <div className="font-display text-2xl text-[#F8F8F2]">{stats.total}</div>
          <div className="text-xs text-[#9CA3AF]">Total</div>
        </div>
        <div className="card-gym text-center">
          <CheckCircle className="w-5 h-5 text-[#22C55E] mx-auto mb-1" />
          <div className="font-display text-2xl text-[#F8F8F2]">{stats.completed}</div>
          <div className="text-xs text-[#9CA3AF]">Payées</div>
        </div>
        <div className="card-gym text-center">
          <Clock className="w-5 h-5 text-[#EAB308] mx-auto mb-1" />
          <div className="font-display text-2xl text-[#F8F8F2]">{stats.open}</div>
          <div className="text-xs text-[#9CA3AF]">En attente</div>
        </div>
        <div className="card-gym text-center">
          <div className="font-display text-2xl text-[#22C55E]">{(stats.revenue / 1000).toFixed(3)}</div>
          <div className="text-xs text-[#9CA3AF]">TND</div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {(['all', 'completed', 'open', 'cancelled'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${filter === f ? 'bg-[#3B82F6] text-white' : 'bg-[#1C1C27] text-[#9CA3AF] hover:text-[#F8F8F2]'}`}>
            {f === 'all' ? 'Toutes' : f === 'completed' ? 'Payées' : f === 'open' ? 'En attente' : 'Annulées'}
          </button>
        ))}
      </div>

      {loading ? <div className="text-center py-12 text-[#9CA3AF]">Chargement...</div> : sorted.length === 0 ? <div className="text-center py-12 text-[#9CA3AF]">Aucune vente</div> : (
        <div className="space-y-2">
          {sorted.map((o) => (
            <div key={o.id} className="flex items-center justify-between p-4 bg-[#1C1C27] rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-[#9CA3AF]">{new Date(o.createdAt).toLocaleDateString('fr')} {new Date(o.createdAt).toLocaleTimeString('fr', { hour: '2-digit', minute: '2-digit' })}</div>
                </div>
              </div>
