          ...
          {orgs.map((org) => {
            ...
            return (
              <button key={org.id} onClick={() => { selectOrg(org); setOpen(false); }}
                className={`w-full text-left px-3 py-2 flex items-center gap-3 text-sm hover:bg-muted transition ${org.id === selectedOrg.id ? 'bg-blue-500/10' : ''}`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isBoutique ? 'bg-blue-500/10' : isGym ? 'bg-orange-500/10' : 'bg-green-500/10'}`}>
                  {isBoutique ? <Store className="w-4 h-4 text-blue-500" /> : isGym ? <LayoutDashboard className="w-4 h-4 text-orange-500" /> : <LayoutDashboard className="w-4 h-4 text-green-500" />}
                </div>
                ...
                {org.id === selectedOrg.id && <span className="text-blue-500">✓</span>}
              </button>
            );
          })}
        </div>
  ...
  const SidebarContent = () => (
    <>
      <div className="flex items-center gap-3 px-4 py-5 border-b border-border">
        ...
        {!collapsed && (
          <div>
            <div className="font-display text-xl text-foreground tracking-wider">{selectedOrg?.name || 'BOUTIQUE'}</div>
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${
                active ? 'bg-blue-500/10 text-blue-500 border-l-2 border-blue-500' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
              }`}>
