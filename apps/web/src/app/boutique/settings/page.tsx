    ...
    <div className="space-y-6">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition ${activeTab === tab.id ? 'bg-[#3B82F6] text-white' : 'text-[#9CA3AF] hover:text-[#F8F8F2] hover:bg-[#1C1C27]'}`}>
      ...
      {activeTab === 'profile' && (
        <div className="card-gym space-y-4">
          <h2 className="font-display text-xl text-[#F8F8F2] tracking-wider">Profil de la boutique</h2>
      ...
      {activeTab === 'notifications' && (
        <div className="card-gym space-y-4">
          ...
          <p className="text-sm text-[#9CA3AF]">Configurez les alertes de stock faible et les rappels clients.</p>
          <div className="space-y-3">
            <label className="flex items-center gap-3 p-3 bg-[#1C1C27] rounded-lg cursor-pointer">
              ...
              <div>
                ...
                <div className="text-xs text-[#9CA3AF]">Notification quand un article atteint ≤ 5 unités</div>
              </div>
            </label>
            <label className="flex items-center gap-3 p-3 bg-[#1C1C27] rounded-lg cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded accent-[#3B82F6]" />
              <div>
                <div className="text-sm text-[#F8F8F2]">Rappel client</div>
                <div className="text-xs text-[#9CA3AF]">Envoyer un message après 30 jours sans visite</div>
              </div>
            </label>
          </div>
        </div>
      ...
      {activeTab === 'danger' && (
        <div className="card-gym border-[#EF4444]/30 space-y-4">
          ...
          <button className="px-4 py-2 bg-[#EF4444] text-white text-sm rounded-lg hover:bg-[#DC2626] transition">Supprimer la boutique</button>
        </div>
      )}
    </div>
  );
