...
export default function BoutiqueDashboard() {
  ...
  const statCards = [
    { label: 'Ventes', value: stats.orders, icon: ShoppingBag, color: 'text-[#3B82F6]' },
    { label: 'Revenu total', value: `${(stats.revenue / 1000).toFixed(3)} TND`, icon: DollarSign, color: 'text-[#22C55E]' },
    { label: 'Articles', value: stats.products, icon: Package, color: 'text-[#3B82F6]' },
    ...
    <div className="space-y-6">
      ...
      {lowStock.length > 0 && (
        <div className="space-y-2">
          <h2 className="font-display text-xl text-[#F8F8F2] tracking-wider">Stock faible</h2>
          {lowStock.map((p: any) => (
            <div key={p.id} className="flex items-center justify-between p-3 bg-[#EAB308]/10 border border-[#EAB308]/20 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-[#9CA3AF]">{p.stockQuantity} restant(s)</div>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="card-gym">
        <h2 className="font-display text-xl text-[#F8F8F2] tracking-wider mb-4">Ventes récentes</h2>
        {recentOrders.length === 0 ? (
          <div className="text-sm text-[#9CA3AF] text-center py-6">Aucune vente</div>
        ) : (
          <div className="space-y-2">
