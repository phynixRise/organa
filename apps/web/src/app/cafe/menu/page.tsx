  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl text-[#F8F8F2] tracking-wider">Menu</h1>
        <button onClick={() => { resetForm(); setShowForm(!showForm); }} className="flex items-center gap-2 px-4 py-2.5 bg-[#22C55E] text-white rounded-lg text-sm font-medium hover:bg-[#16A34A] transition">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card-gym space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <input type="number" step="0.001" placeholder="Prix (TND) *" value={form.price} onChange={(e) => setForm((p) => ({ ...p, price: e.target.value }))} className="px-3 py-2 bg-[#0A0A0F] border border-white/5 rounded-lg text-sm text-[#F8F8F2] placeholder-[#9CA3AF]" required />
            ...
            <input type="text" placeholder="Description" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} className="px-3 py-2 bg-[#0A0A0F] border border-white/5 rounded-lg text-sm text-[#F8F8F2] placeholder-[#9CA3AF]" />
          </div>
          <button type="submit" className="px-4 py-2 bg-[#22C55E] text-white text-sm rounded-lg hover:bg-[#16A34A] transition">{editingProduct ? 'Modifier' : 'Ajouter au menu'}</button>
        </form>
      )}

      {loading ? <div className="text-center py-12 text-[#9CA3AF]">Chargement...</div> : filtered.length === 0 ? <div className="text-center py-12 text-[#9CA3AF]">{search ? 'Aucun résultat' : 'Menu vide'}</div> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((p) => (
            <div key={p.id} className="card-gym hover:border-[#22C55E]/30 transition-colors">
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 bg-[#22C55E]/10 rounded-lg flex items-center justify-center"><Grid3X3 className="w-5 h-5 text-[#22C55E]" /></div>
                <div className="flex gap-1">
                  <button onClick={() => startEdit(p)} className="text-[#6B7280] hover:text-[#22C55E] transition"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(p.id)} className="text-[#6B7280] hover:text-[#EF4444] transition"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
              <h3 className="text-sm font-medium text-[#F8F8F2] mb-1">{p.name}</h3>
              <div className="font-display text-2xl text-[#22C55E] tracking-wide mb-2">{(p.priceMillimes / 1000).toFixed(3)} <span className="text-sm">TND</span></div>
              {p.category && <div className="flex items-center gap-1 text-xs text-[#9CA3AF]"><Tag className="w-3 h-3" />{p.category}</div>}
              {p.description && <div className="text-xs text-[#6B7280] mt-1 line-clamp-2">{p.description}</div>}
            </div>
          ))}
        </div>
      )}
