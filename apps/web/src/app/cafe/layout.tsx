          ...
          {orgs.map((org) => {
            ...
            return (
              <button key={org.id} onClick={() => { selectOrg(org); setOpen(false); }}
                className={`w-full text-left px-3 py-2 flex items-center gap-3 text-sm hover:bg-[#1C1C27] transition ${org.id === selectedOrg.id ? 'bg-[#22C55E]/10' : ''}`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isCafe ? 'bg-[#22C55E]/10' : isGym ? 'bg-[#F97316]/10' : 'bg-[#3B82F6]/10'}`}>
                  {isCafe ? <Coffee className="w-4 h-4 text-[#22C55E]" /> : isGym ? <LayoutDashboard className="w-4 h-4 text-[#F97316]" /> : <LayoutDashboard className="w-4 h-4 text-[#3B82F6]" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[#F8F8F2] font-medium truncate">{org.name}</div>
                  <div className="text-xs text-[#9CA3AF]">{org.businessType}</div>
                </div>
                {org.id === selectedOrg.id && <span className="text-[#22C55E]">✓</span>}
              </button>
            );
          })}
        </div>
  ...
  const SidebarContent = () => (
    <>
      <div className="flex items-center gap-3 px-4 py-5 border-b border-white/5">
        ...
        {!collapsed && (
          <div>
            <div className="font-display text-xl text-[#F8F8F2] tracking-wider">{selectedOrg?.name || 'CAFÉ'}</div>
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${
                active ? 'bg-[#22C55E]/10 text-[#22C55E] border-l-2 border-[#22C55E]' : 'text-[#9CA3AF] hover:bg-[#1C1C27] hover:text-[#F8F8F2]'
              }`}>
