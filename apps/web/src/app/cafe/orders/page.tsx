...
export default function CafeOrders() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-display text-3xl text-foreground tracking-wider">Commandes</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="card-gym text-center">
          ...
          <div className="text-xs text-muted-foreground">Servies</div>
        </div>
        <div className="card-gym text-center">
      ...
      <div className="flex gap-2">
        {(['all', 'completed', 'open', 'cancelled'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${filter === f ? 'bg-green-500 text-white' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
            {f === 'all' ? 'Toutes' : f === 'completed' ? 'Servies' : f === 'open' ? 'En attente' : 'Annulées'}
          </button>
        ))}
      </div>

      {loading ? <div className="text-center py-12 text-muted-foreground">Chargement...</div> : sorted.length === 0 ? <div className="text-center py-12 text-muted-foreground">Aucune commande</div> : (
