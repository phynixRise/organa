    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="card-gym text-center">
          <ClipboardList className="w-5 h-5 text-[#22C55E] mx-auto mb-1" />
        ...
        <div className="card-gym text-center">
          ...
          <div className="text-xs text-[#9CA3AF]">Servies</div>
        </div>
        <div className="card-gym text-center">
      ...
      <div className="flex gap-2">
        {(['all', 'completed', 'open', 'cancelled'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${filter === f ? 'bg-[#22C55E] text-white' : 'bg-[#1C1C27] text-[#9CA3AF] hover:text-[#F8F8F2]'}`}>
            {f === 'all' ? 'Toutes' : f === 'completed' ? 'Servies' : f === 'open' ? 'En attente' : 'Annulées'}
          </button>
        ))}
      </div>

      {loading ? <div className="text-center py-12 text-[#9CA3AF]">Chargement...</div> : sorted.length === 0 ? <div className="text-center py-12 text-[#9CA3AF]">Aucune commande</div> : (
