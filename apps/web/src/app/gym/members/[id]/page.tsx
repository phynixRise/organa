...
export default function MemberDetailPage() {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen bg-background"><div className="text-muted-foreground">Chargement...</div></div>;
  if (!member) return <div className="flex items-center justify-center min-h-screen bg-background"><div className="text-[#EF4444]">Membre non trouvé</div></div>;
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => router.back()} className="p-2 bg-card border border-border rounded-lg hover:bg-muted transition">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl text-foreground tracking-wider">{member.name}</h1>
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="card-gym lg:col-span-2 space-y-4">
            <h2 className="font-display text-xl text-foreground tracking-wider">Informations</h2>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="text-xs text-muted-foreground">Nom</label><div className="text-sm text-foreground">{member.name}</div></div>
              <div><label className="text-xs text-muted-foreground">Email</label><div className="text-sm text-foreground">{member.email || '—'}</div></div>
              <div><label className="text-xs text-muted-foreground">Téléphone</label><div className="text-sm text-foreground">{member.phone || '—'}</div></div>
              <div><label className="text-xs text-muted-foreground">Code-barres</label><div className="text-sm text-foreground font-mono">{member.barcode || '—'}</div></div>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="card-gym text-center">
              <div className="text-xs text-muted-foreground mb-1">Commandes</div>
              <div className="font-display text-3xl text-foreground">{orders.length}</div>
            </div>
            <div className="card-gym text-center">
              <div className="text-xs text-muted-foreground mb-1">Présences</div>
              <div className="font-display text-3xl text-brand-teal dark:text-brand-cyan">{appointments.length}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'payments' && (
        <div className="card-gym">
          <h2 className="font-display text-xl text-foreground tracking-wider mb-4">Historique des paiements</h2>
          {orders.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">Aucun paiement</div>
          ) : (
      ...
      {activeTab === 'attendance' && (
        <div className="card-gym">
          <h2 className="font-display text-xl text-foreground tracking-wider mb-4">Historique de présence</h2>
              ...
              {appointments.map((a) => (
                <div key={a.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm text-foreground">{new Date(a.startTime).toLocaleDateString('fr')}</div>
      ...
      {activeTab === 'qr' && (
        <div className="card-gym text-center space-y-4">
          <h2 className="font-display text-xl text-foreground tracking-wider">QR Code du membre</h2>
