...
function OrgSwitcherGym() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="flex items-center gap-2 px-3 py-1.5 bg-muted border border-border rounded-lg hover:bg-[#22222E] text-sm transition">
        <Dumbbell className="w-4 h-4 text-brand-teal dark:text-brand-cyan" />
          ...
          {orgs.map((org) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 py-2 flex items-center gap-3 text-sm hover:bg-muted transition ${org.id === selectedOrg.id ? 'bg-brand-teal/10 dark:bg-brand-cyan/10' : ''}`}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isGym ? 'bg-brand-teal/10 dark:bg-brand-cyan/10' : 'bg-[#3B82F6]/10'}`}>
                  {isGym ? <Dumbbell className="w-4 h-4 text-brand-teal dark:text-brand-cyan" /> : <LayoutDashboard className="w-4 h-4 text-[#3B82F6]" />}
                </div>
                ...
                {org.id === selectedOrg.id && <span className="text-brand-teal dark:text-brand-cyan">✓</span>}
              </button>
            );
          })}
        </div>
  ...
  const SidebarContent = () => (
    <>
      <div className="flex items-center gap-3 px-4 py-5 border-b border-border">
        ...
        {!collapsed && (
          <div>
            <div className="font-display text-xl text-foreground tracking-wider">
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${
                active
                  ? 'bg-brand-teal/10 dark:bg-brand-cyan/10 text-brand-teal dark:text-brand-cyan border-l-2 border-brand-teal dark:border-brand-cyan'
      ...
      <div className="px-3 py-4 border-t border-border">
        {!collapsed && (
          <div className="text-xs text-muted-foreground px-3 mb-2 truncate">{account.email}</div>
        )}
        <button
          onClick={logout}
    ...
    <div className="flex min-h-screen bg-background">
      <aside className={`hidden lg:flex flex-col ${collapsed ? 'w-[72px]' : 'w-[240px]'} bg-card border-r border-border transition-all duration-200`}>
        ...
        <button
          ...
          className="p-3 border-t border-border text-muted-foreground hover:text-foreground hover:bg-muted transition"
        >
